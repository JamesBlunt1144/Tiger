...
exports.getClientsByFullName = async (req, res) => {
    const { client } = req.query; // URL dan client parametrini olish

    if (!client) {
        return res.status(400).json({
            success: false,
            message: 'Qidiruv uchun client parametr ko\'rsatilmagan.' // Bu yerda 'Full_name' emas, 'client' bo'lishi kerak
        });
    }

    try {
        const knex = await Client.knex();
        const result = await knex('client')
            .where('Full_name', 'like', `%${client}%`); // Full_name ustunida qidirish
